...
(define (real-part-polar z) (* (cos (angle-polar z)) (magnitude-polar a)))
(define (imag-part-polar z) (* (sin (angle-polar z)) (magnitude-polar a)))
...
;; now, we only need a way to make our constructors generic too. we can, of course, construct a complex number in rectangular form when we get magnitude and angle, and vice versa, but most naturally will be if we construct a square-representation when given numbers that mean real and imaginary, and polar when given numbers that mean angle and magnitude
